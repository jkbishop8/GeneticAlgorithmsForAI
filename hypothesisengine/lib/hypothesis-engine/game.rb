module HypothesisEngine
  class Game
    ...
    def start
	  if not Config.quiet
         UI.puts "Welcome to Hypothesis Engine"
	  end
      
      if File.exist?(Config.path_prefix + '/.profile')
        @profile = Profile.load(Config.path_prefix + '/.profile')
      else
        if File.exist?(Config.path_prefix + '/hyopothesis-engine')
          FileUtils.mv(Config.path_prefix + '/hypothesis-engine', Config.path_prefix + '/hypothesisengine')
        end
        make_game_directory unless File.exist?(Config.path_prefix + '/hypothesisengine')
      end
    ...
    def make_game_directory
      if UI.ask("No hypothesisengine directory found, would you like to create one?")
        Dir.mkdir(Config.path_prefix + '/hypothesisengine')
      else
    ...
    def play_normal_mode
      if Config.practice_level
        ...
        if current_level.number.zero?
          prepare_next_level
          UI.puts "First level has been generated. See the hypothesisengine/#{profile.directory_name}/README for instructions."
        else
          play_current_level
        end
      end
    ...
    def request_next_level
      if !Config.skip_input?
        if (next_level.exists? ? UI.ask("Would you like to continue on to the next level?") : UI.ask("Would you like to continue on to epic mode?"))
			if next_level.exists?
			  prepare_next_level
			  UI.puts "See the updated README in the hypothesisengine/#{profile.directory_name} directory."
			else
			  prepare_epic_mode
			  UI.puts "Run hypothesisengine again to play epic mode."
			end
		  else
    ...
    def profile_paths
      Dir[Config.path_prefix + '/hypothesisengine/**/.profile']
    end
    ...
    def create_profile
      profile = Profile.new
      profile.tower_path = Config.create_tower
      profile.warrior_name = Config.create_name
      profile
    end	
